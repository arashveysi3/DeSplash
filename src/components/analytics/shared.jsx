...
export function StatTile({ icon: Icon, value, label, sub, color, delay = 0 }) {
  return (
    <div
    ...
    >
      {Icon && (
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 4 }}>
          <Icon size={ICON_SIZES.card} aria-hidden="true" style={{ color: color || '#0f0f12' }} />
        </div>
      )}
    ...
    <div className="gs-report-item" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, animationDelay: `${delay}ms` }}>
      {strongest && (
        <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 14, padding: '10px 12px' }}>
          <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: 0.6, color: '#16a34a', display: 'flex', alignItems: 'center', gap: 4 }}><CheckCircle2 size={12} aria-hidden="true" /> GOING WELL</div>
...
export function RecommendationBox({ recommendation, delay = 0 }) {
  ...
  return (
    ...
    >
      <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: 0.8, color: low ? '#6b6b7a' : 'rgba(255,255,255,0.7)', display: 'flex', alignItems: 'center', gap: 4 }}>
        {!low && <Sparkles size={12} aria-hidden="true" />}
...
export function AnalyticsEmpty({ icon: Icon = BarChart3, title, hint }) {
  return (
    <UberCard styleOverride={{ textAlign: 'center', paddingTop: 24, paddingBottom: 24, backgroundColor: '#f7f7fb', borderColor: '#e9e8f0' }}>
      <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 4 }}>
        <Icon size={ICON_SIZES.empty} aria-hidden="true" style={{ color: '#9aa0b2' }} />
      </div>
  ...
  return (
    <UberCard styleOverride={{ textAlign: 'center', backgroundColor: '#fef2f2', borderColor: '#fecaca' }}>
      <div style={{ display: 'flex', justifyContent: 'center' }}>
        <TriangleAlert size={28} aria-hidden="true" style={{ color: '#dc2626' }} />
      </div>
